...
class YOLO(object):
    _defaults = {
        "model_path": 'logs/ep063-loss5.490-val_loss3.299.h5',
    ...
    def detect_image_without_drawing(self, image):
        ...
        for i, c in reversed(list(enumerate(out_classes))):
            ...
            # add everything to list
            ObjectsList.append([label, scores, left, top, width, height])
    ...
    def detect_img_without_drawing(self, image):
        image = cv2.imread(image, cv2.IMREAD_COLOR)
        original_image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        original_image_color = cv2.cvtColor(original_image, cv2.COLOR_BGR2RGB)

        ObjectsList = self.detect_image_without_drawing(original_image_color)
        return ObjectsList
